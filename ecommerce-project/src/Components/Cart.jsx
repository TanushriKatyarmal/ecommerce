            ...
            <table border={2} className='table'>
                ...
                <tbody>
                    {cart.map((item)=>{
                        return(
                            <tr key={item.id}>
                                <td id='image'><Link to={`/about/${item.id}`}><img src={item.image} height={'50px'} width={'50px'} alt=""  /></Link></td>
                                <td>{item.title}</td>
                                <td><button onClick={()=>inc(item.id)} >+</button></td>
                                <td style={{fontSize:'19px'}}>{item.amount}</td>
                                <td><button onClick={()=>dec(item.id)}>-</button></td>
                                <td style={{fontSize:'19px', width:'40px'}}>${(item.price * item.amount).toFixed(2)}</td>
                ...
                <tfoot>
                    <tr>
                        <td>Total Amount</td>
                        <td colSpan={5} style={{fontSize:'18px'}}><b>${price.toFixed(2)}</b></td>
                        <td>
                            <button className='paybtn' id='gpay'>
                            <GooglePayButton
                            environment="TEST"
                            paymentRequest={{
                            apiVersion: 2,
